...
import {  getTenantUrlsSync, getTenantContactSync, getTenantIdentitySync, getTenantLegalRepresentativeSync, getActiveCompanyLegalDataSync } from "../core/configHelpers.js";
...
function AnexosPanel({p}){
  ...
  // Auto-include digitally generated documents
  const autoGenerated = {};
  if(p._condiciones?.html_trabajador) autoGenerated.condiciones_trabajador = {nombre:"Generado digitalmente",auto:true,fecha:p._condiciones.generado};
  if(p._condiciones?.html_empleador) autoGenerated.condiciones_empleador = {nombre:"Generado digitalmente",auto:true,fecha:p._condiciones.generado};
  const [docs, setDocs] = React.useState({...autoGenerated,...(p._expediente||{})});
  const [expanded, setExpanded] = React.useState(true);
  const [dragKey, setDragKey] = React.useState(null);

  // Open auto-generated condition docs
  const abrirCondicion = (tipo) => {
    const html = tipo==="trabajador" ? p._condiciones?.html_trabajador : p._condiciones?.html_empleador;
    if(html) window.open(URL.createObjectURL(new Blob([html],{type:'text/html;charset=utf-8'})),'_blank');
    else alert("Documento no disponible — regenera desde la calculadora");
  };
  ...
  const CATS = [
    ...
    {id:"emp_contrato",section:"empresa",label:"Documentos contractuales",icon:"📄",color:"#1E6B42",docs:[
      {key:"condiciones_trabajador",label:"Condiciones laborales — trabajador",gen:()=>abrirCondicion('trabajador')},
      {key:"condiciones_empleador",label:"Condiciones laborales — empleador",gen:()=>abrirCondicion('empleador')},
  ...
  return (
    <div style={sCard}>
                  ...
                  {cat.docs.map(d=>{
                    ...
                    return <div key={d.key}>
                      <div style={{display:'flex',alignItems:'center',gap:8,padding:'4px 8px',borderBottom:'1px solid #F0EEE9',background:uploaded?(isPending?'#FFFBEB':'#FAFFF9'):'transparent'}}>
                        ...
                        {uploaded && !isPending && !uploaded.auto && <>
                          <span style={{fontSize:9,color:'#1E6B42'}}>{uploaded.nombre}</span>
                          {uploaded.data && <button onClick={(ev)=>{ev.stopPropagation();setViewDoc({label:d.label,data:uploaded.data,nombre:uploaded.nombre});}} style={{...btnS,background:'#EFF6FF',color:'#2563EB',border:'1px solid #2563EB'}}>👁 Ver</button>}
      ...
      {viewDoc && (
        <div onClick={()=>setViewDoc(null)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,.7)',zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center',padding:20}}>
          <div onClick={e=>e.stopPropagation()} style={{background:'#fff',borderRadius:12,overflow:'hidden',maxWidth:'90vw',maxHeight:'90vh',width:800,display:'flex',flexDirection:'column',boxShadow:'0 8px 40px rgba(0,0,0,.3)'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'12px 16px',borderBottom:'1px solid #E5E3DE',background:'#FAFAF8'}}>
              <div>
                <div style={{fontSize:13,fontWeight:700}}>{viewDoc.label}</div>
                <div style={{fontSize:10,color:'#999'}}>{viewDoc.nombre}</div>
              </div>
              <div style={{display:'flex',gap:6}}>
                <a href={viewDoc.data} download={viewDoc.nombre} style={{padding:'5px 12px',fontSize:11,fontWeight:600,border:'1px solid #1E6B42',borderRadius:6,background:'#E8F4EE',color:'#1E6B42',textDecoration:'none',cursor:'pointer'}}>↓ Descargar</a>
                <button onClick={()=>setViewDoc(null)} style={{padding:'5px 12px',fontSize:11,fontWeight:600,border:'1px solid #dc2626',borderRadius:6,background:'#FEF2F2',color:'#dc2626',cursor:'pointer',fontFamily:'DM Sans,sans-serif'}}>✕ Cerrar</button>
              </div>
            </div>
            <div style={{flex:1,overflow:'auto',padding:16,display:'flex',alignItems:'center',justifyContent:'center',minHeight:400,background:'#F0EEE9'}}>
              {viewDoc.data.startsWith('data:image')
                ? <img src={viewDoc.data} alt={viewDoc.label} style={{maxWidth:'100%',maxHeight:'70vh',objectFit:'contain',borderRadius:4,boxShadow:'0 2px 8px rgba(0,0,0,.1)'}}/>
                : viewDoc.data.startsWith('data:application/pdf')
                  ? <iframe src={viewDoc.data} style={{width:'100%',height:'70vh',border:'none',borderRadius:4}} title={viewDoc.label}/>
                  : <div style={{textAlign:'center',padding:40}}>
                      <div style={{fontSize:40,marginBottom:12}}>📄</div>
                      <div style={{fontSize:13,color:'#666'}}>Vista previa no disponible para este tipo de archivo</div>
                      <a href={viewDoc.data} download={viewDoc.nombre} style={{display:'inline-block',marginTop:12,padding:'8px 20px',fontSize:12,fontWeight:600,border:'1px solid #1E6B42',borderRadius:6,background:'#E8F4EE',color:'#1E6B42',textDecoration:'none'}}>↓ Descargar archivo</a>
                    </div>
              }
            </div>
          </div>
          ...
          <Card key={emp.id} style={{marginBottom:12,border:isOpen?"2px solid #1E6B42":alertaVenc?"2px solid #D97706":vencido?"2px solid #dc2626":"1px solid "+C.border}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",cursor:"pointer"}} onClick={()=>setSelEmp(isOpen?null:emp.id)}>
              <div style={{display:"flex",gap:12,alignItems:"center"}}>
                {/* Foto perfil */}
                <div style={{width:48,height:48,borderRadius:"50%",overflow:"hidden",border:`2px solid ${C.border}`,background:"#F5F4F1",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
                  {emp._expediente?.foto_perfil?.data
                    ? <img src={emp._expediente.foto_perfil.data} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                    : <span style={{fontSize:20,color:C.inkLight}}>👤</span>
                  }
                </div>
                <div>
            ...
            {isOpen && (
              <div style={{marginTop:14,borderTop:"1px solid "+C.border,paddingTop:14}}>
                {/* Foto de perfil + Info personal */}
                <div style={{display:"flex",gap:14,marginBottom:14}}>
                  {/* Foto grande */}
                  <div style={{flexShrink:0,textAlign:"center"}}>
                    <div style={{width:100,height:120,borderRadius:8,overflow:"hidden",border:`2px solid ${C.border}`,background:"#F5F4F1",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:6}}>
                      {emp._expediente?.foto_perfil?.data
                        ? <img src={emp._expediente.foto_perfil.data} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                        : <span style={{fontSize:36,color:C.inkXLight}}>👤</span>
                      }
                    </div>
                    <button onClick={(ev)=>{ev.stopPropagation();const input=document.createElement('input');input.type='file';input.accept='image/*';input.onchange=(e)=>{const file=e.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=(r)=>{const nd={...(emp._expediente||{}),foto_perfil:{data:r.target.result,nombre:file.name,fecha:new Date().toISOString()}};fetch("/api/hiring",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:emp.id,_expediente:nd})}).then(()=>loadEmpleados());};reader.readAsDataURL(file);};input.click();}} style={{padding:"3px 10px",fontSize:9,fontWeight:600,border:"1px solid "+C.border,borderRadius:4,background:C.bg,cursor:"pointer",fontFamily:"DM Sans,sans-serif",color:C.inkMid}}>📷 {emp._expediente?.foto_perfil?"Cambiar":"Subir foto"}</button>
                  </div>
                  {/* Info cards */}
                ...
                <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}>
                  ...
                  {[true,false].map(conSal=>(
                    <button key={conSal?"con":"sin"} onClick={()=>{
                      ...
                      const bodyHtml=`<h1>CERTIFICACIÓN LABORAL</h1><div class="body"><p>El suscrito representante legal de <b>${getActiveCompanyLegalDataSync().legalName}</b>, NIT ${getActiveCompanyLegalDataSync().taxId}, certifica que:</p><br/><p><b>${emp.candidato_nombre||""}</b>, identificado(a) con ${emp.tipo_documento||"C.C."} No. <b>${emp.candidato_cc||""}</b>, labora en esta empresa desde el <b>${emp.fecha_inicio?new Date(emp.fecha_inicio+"T12:00:00").toLocaleDateString(getTenantDefaultsSync().locale,{day:"numeric",month:"long",year:"numeric"}):"—"}</b>, desempeñando el cargo de <b>${emp.cargo||""}</b> en el área <b>${emp.area||"Administrativa"}</b>.</p><br/>${salText}<p>La presente certificación se expide a solicitud del interesado, en Bogotá D.C., a los ${hoyStr}.</p></div><div class="sig"><div><br/><b>Representante Legal</b><br/>${getActiveCompanyLegalDataSync().legalName}</div></div><div class="foot">Habitaris Suite · ${hoyStr}</div>`;
                      const fullHtml=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${fileName}</title>
<script src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js"><\/script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js"><\/script>
<style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Helvetica,Arial,sans-serif;background:#e5e5e5;padding:20px 0}#content{background:#fff;width:794px;margin:0 auto;padding:35px 45px;font-size:9pt;color:#111;line-height:1.45;box-shadow:0 0 8px rgba(0,0,0,.15)}.hdr{border-bottom:2px solid #111;padding-bottom:8px;margin-bottom:14px;overflow:hidden}.hdr .l{float:left}.hdr .r{float:right;text-align:right;font-size:8pt;color:#666;padding-top:6px}.hdr img{height:36px}h1{font-size:13pt;text-align:center;margin:16px 0 12px;clear:both}.body{font-size:10.5pt;line-height:1.7;text-align:justify}.sig{margin-top:50px;width:50%}.sig div{border-top:1px solid #111;padding-top:6px;font-size:9pt}.foot{font-size:6pt;color:#999;text-align:center;margin-top:20px;clear:both}.np{text-align:center;margin:16px auto;max-width:794px}.btn{background:#111;color:#fff;border:none;padding:10px 24px;border-radius:4px;cursor:pointer;font-size:11pt;font-weight:600;margin:0 4px}.btn2{background:#fff;color:#111;border:1px solid #111;padding:10px 24px;border-radius:4px;cursor:pointer;font-size:11pt;margin:0 4px}@media print{body{background:#fff;padding:0}.np{display:none}#content{width:100%;margin:0;padding:0;box-shadow:none}}</style></head><body><div id="content"><div class="hdr"><div class="l"><img src="${HAB_LOGO}" alt="Habitaris"/></div><div class="r"><div style="font-weight:600;color:#111">${getActiveCompanyLegalDataSync().legalName}</div><div>NIT: ${getActiveCompanyLegalDataSync().taxId}</div></div></div>${bodyHtml}</div><div class="np"><button class="btn" onclick="(function(){var el=document.getElementById('content');el.style.boxShadow='none';document.querySelector('.np').style.display='none';html2canvas(el,{scale:2,useCORS:true,width:el.scrollWidth,windowWidth:el.scrollWidth,backgroundColor:'#fff'}).then(function(c){var img=c.toDataURL('image/jpeg',0.98);var pW=210,pH=(c.height*pW)/c.width;var pdf=new jspdf.jsPDF({orientation:'portrait',unit:'mm',format:'a4'});if(pH<=297){pdf.addImage(img,'JPEG',0,0,pW,pH)}else{var pos=0,pg=0;while(pos<pH){if(pg>0)pdf.addPage();pdf.addImage(img,'JPEG',0,-pos,pW,pH);pos+=297;pg++}}pdf.save('${fileName}.pdf');el.style.boxShadow='0 0 8px rgba(0,0,0,.15)';document.querySelector('.np').style.display=''})})()">📥 Descargar PDF</button><button class="btn2" onclick="window.print()">🖨️ Imprimir</button></div></body></html>`;
                      const w=window.open('','_blank');w.document.write(fullHtml);w.document.close();
                    }} style={{padding:"5px 12px",fontSize:11,fontWeight:600,border:`1px solid ${conSal?"#0891B2":"#999"}`,borderRadius:6,background:conSal?"#E0F2FE":"#f5f5f5",cursor:"pointer",fontFamily:"DM Sans,sans-serif",color:conSal?"#0891B2":"#666"}}>
                ...
                {/* Mini-dashboard anual del trabajador */}
                <DashboardTrabajadorAnio empId={emp.id} fechaIngreso={emp.fecha_inicio} anio={new Date().getFullYear()} />
...
const TABS = [
  ...
  { id:"_sep2", lbl:"PERSONAL", sep:true },
  ...
  { id:"_sep5", lbl:"ADMINISTRACIÓN", sep:true },
      ...
      <div style={{ height:"100vh", display:"flex", flexDirection:"column", background:T.bg, overflow:"hidden" }}>
        ...
        <div style={{ display:"flex", flex:1, minHeight:0 }}>
          ...
          <div style={{ width:210, flexShrink:0, background:T.surface, borderRight:`1px solid ${T.border}`,
            ...
            {TABS.map(t => {
              if(t.sep) return (
                <div key={t.id} style={{padding:"12px 16px 4px",fontSize:9,fontWeight:800,color:T.inkLight,letterSpacing:1.2,textTransform:"uppercase"}}>{t.lbl}</div>
              );
              const act = tab === t.id;
              return (
