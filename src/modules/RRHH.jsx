                  ...
                  <div style={{padding:"0 18px 16px",borderTop:"1px solid "+C.border}}>
                    ...
                    <div style={{display:"flex",gap:6,flexWrap:"wrap",paddingTop:8,borderTop:"1px solid "+C.border}}>
                      ...
                      {(p.estado==="psicotecnico") && <>
                        <button onClick={async()=>{const ar=await fetch("/api/aprobar",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({hiring_id:p.id,tipo:"examen_medico",aprobador_nombre:"",aprobador_email:""})});const ad=await ar.json();if(ad.ok){navigator.clipboard.writeText(ad.link);alert("Link de aprobación examen médico copiado:\n"+ad.link);}var psiLink="https://suite.habitaris.co/psicotecnico?hiring_id="+p.id;setSendModal({link:psiLink,email:p.candidato_email,titulo:"Evaluaci\u00f3n psicot\u00e9cnica",nombre:p.candidato_nombre,hiringId:p.id,nextEstado:"psicotecnico"}); style={{padding:"6px 12px",fontSize:11,fontWeight:600,border:"1px solid #0D5E6E",borderRadius:6,background:"#E0F2FE",cursor:"pointer",fontFamily:"DM Sans,sans-serif",color:"#0D5E6E"}}>🏥 Psicotécnico OK → Enviar a examen médico</button>
                      </>}
