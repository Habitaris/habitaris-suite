...
const C = { ink:"#111",inkMid:"#444",inkLight:"#888",inkXLight:"#C8C5BE",bg:"#F5F4F1",surface:"#FFFFFF",border:"#E4E1DB",green:"#1E6B42",greenBg:"#E8F4EE",greenLight:"#D4EDDF",red:"#AE2C2C",redBg:"#FAE8E8",amber:"#7A5218",amberBg:"#FFF8EE",blue:"#1E4F8C",blueBg:"#E6EFF9",shadow:"0 1px 4px rgba(0,0,0,.06), 0 8px 32px rgba(0,0,0,.08)" };
...
function KPI({label,value,highlight}){return(<div style={{flex:1,minWidth:140,background:highlight?C.greenBg:"#FAFAF8",borderRadius:10,padding:"14px 16px",border:"1px solid "+(highlight?C.greenLight:C.border)}}><p style={{...F,margin:"0 0 4px",fontSize:9,fontWeight:600,textTransform:"uppercase",letterSpacing:1,color:C.inkLight}}>{label}</p><p style={{...F,margin:0,fontSize:18,fontWeight:700,color:highlight?C.green:C.ink}}>{value}</p></div>);}
...
export default function OfferApproval({ token }) {
  ...
  return (
    <div style={{...F,minHeight:"100vh",background:C.bg}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700;800&display=swap');*{box-sizing:border-box;margin:0;padding:0}body{font-family:'Outfit',sans-serif}`}</style>
      ...
      <div style={{maxWidth:760,margin:"0 auto",padding:"32px 24px 120px"}}>
        ...
        {d.notaLegal && <div style={{background:"#FAFAF8",borderRadius:10,padding:16,marginBottom:28,border:"1px solid "+C.border}}><p style={{fontSize:10,fontWeight:600,textTransform:"uppercase",letterSpacing:1,color:C.inkLight,margin:"0 0 6px"}}>Nota legal</p><p style={{fontSize:11,color:C.inkMid,lineHeight:1.6,margin:0}}>{d.notaLegal}</p></div>}
        <div style={{background:C.surface,borderRadius:16,padding:28,boxShadow:C.shadow,border:"2px solid "+C.border}}>
          ...
          {action && <div style={{marginBottom:20}}><label style={{display:"block",fontSize:11,fontWeight:600,color:C.ink,marginBottom:6}}>{action==="devolver"?"Describa los cambios que necesita *":"Comentarios (opcional)"}</label><textarea value={comments} onChange={e=>setComments(e.target.value)} placeholder={action==="devolver"?"Indique qué ajustes necesita...":"Puede añadir notas..."} rows={4} style={{...F,width:"100%",padding:12,borderRadius:8,border:"1px solid "+C.border,fontSize:13,resize:"vertical",background:"#FAFAF8"}} /></div>}
