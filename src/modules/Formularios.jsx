...
import TabPublicos from "./TabPublicos.jsx";
...
const PLANTILLAS = [
  { id:"briefing_inicial", version:6, nombre:"Briefing Inicial Habitaris", modulo:"crm", desc:"Formulario completo de briefing para nuevos proyectos",
    campos:[
      ...
      {id:"f_forma_pago_co",tipo:"select",label:"Forma de pago preferida",placeholder:"",required:true,opciones:["Enlace PSE","Tarjeta de Crédito/Débito"],logica:{fieldId:"f_pais",value:"Colombia"},mapKey:"formaPago"},
      {id:"f_forma_pago_es",tipo:"select",label:"Forma de pago preferida",placeholder:"",required:true,opciones:["Tarjeta de Crédito/Débito","Transferencia bancaria","PayPal"],logica:{fieldId:"f_pais",value:"España"},mapKey:"formaPago"},
      ...
      {showShare && createPortal((
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:9999,display:"flex",alignItems:"flex-start",justifyContent:"center",paddingTop:40,overflowY:"auto"}} onClick={closeShare}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:8,padding:28,width:460,maxHeight:"85vh",overflowY:"auto",boxShadow:"0 12px 40px rgba(0,0,0,0.2)",marginBottom:40}}>
            ...
            {/* Share actions — flujo unificado: link público, sin descarga HTML legacy */}
            <div style={{display:"flex",flexDirection:"column",gap:8,opacity:shareGenerated?1:.4,pointerEvents:shareGenerated?"auto":"none"}}>
              {sharePublicUrl && (
                <button onClick={()=>{navigator.clipboard.writeText(sharePublicUrl);window.toast?.("✅ Link copiado al portapapeles","success");}} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",border:"none",borderRadius:6,background:"#111",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",textAlign:"left",color:"#fff"}}>
                  <div style={{width:32,height:32,borderRadius:6,background:"rgba(255,255,255,0.15)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Copy size={14} color="#fff"/></div>
        ...
        <table style={{borderCollapse:"collapse",width:"100%"}}>
          ...
          <tbody>
            {filtered.length===0 ? (
              ...
              const hasResp = respuestas.some(r=>(r.link_id||r.linkId)===e.linkId && (!e.rehabilitatedAt || (r.fecha && new Date(r.fecha) > new Date(e.rehabilitatedAt))));
...
// =================================================================
// TabPublicos - Modulo central para gestionar formularios publicos
// Permite ver, configurar, activar/desactivar y copiar el link de
// cada formulario que se puede solicitar publicamente.
// =================================================================
