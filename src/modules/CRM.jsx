...
const TEMPLATE = {
  ...
  tipoProyecto: "OBRA", subtipoDiseno: SUBTIPO[1], servicioId: "interiorismo",
  ...
  costosDirectos: [], entregablesOferta: [],
...
function FormBuilder() {
  ...
  useEffect(() => {
    if (!sharing) { setSharingUrl(""); return; }
    (async () => {
      const url = await getFormUrl(sharing);
      setSharingUrl(url || "Error generando URL");
    })();
  }, [sharing]);
    ...
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.5)", zIndex:500, display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}
      ...
      <div style={{ background:"#fff", borderRadius:10, maxWidth:520, width:"100%", padding:24, boxShadow:"0 20px 60px rgba(0,0,0,.2)" }}>
        ...
        <div style={{ marginBottom:16, textAlign:"center" }}>
          <a href={sharingUrl || "Generando..."} target="_blank" rel="noreferrer"
...
function TabClientesCRM() {
  ...
  useEffect(() => {
    (() => { try { const r = store.getSync(SKEY); if(r) {
