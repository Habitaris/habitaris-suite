...
// Estimación de la nómina del mes (devengado, deducciones, neto, anticipo Q1 y Q2).
// Compartido por el Informe de novedades y la Liquidación provisional de prima,
// para que el desglose sea idéntico en ambos y cambie en un solo sitio.
export function nominaEstimacionHtml(calc, isQ, f = fmt) {
  if (!calc) return "";
  return `<h2>Estimación de la nómina</h2>
...
<tr><td>Salario (${calc.dias} d)</td><td style="text-align:right;font-family:'DM Mono',monospace">${f(calc.salProp)}</td><td style="text-align:right">—</td></tr>
${calc.aux>0?`<tr><td>Auxilio de transporte (${calc.diasComm} d)</td><td style="text-align:right;font-family:'DM Mono',monospace">${f(calc.aux)}</td><td style="text-align:right">—</td></tr>`:""}
${calc.bono>0?`<tr><td>Bono de asistencia (${calc.diasAsist} d)</td><td style="text-align:right;font-family:'DM Mono',monospace">${f(calc.bono)}</td><td style="text-align:right">—</td></tr>`:""}
${(calc.totHex+calc.recFest)>0?`<tr><td>Horas extra y recargos</td><td style="text-align:right;font-family:'DM Mono',monospace">${f(calc.totHex+calc.recFest)}</td><td style="text-align:right">—</td></tr>`:""}
<tr><td>Salud (EPS) 4%</td><td style="text-align:right">—</td><td style="text-align:right;font-family:'DM Mono',monospace">${f(calc.epsE)}</td></tr>
<tr><td>Pensión 4%</td><td style="text-align:right">—</td><td style="text-align:right;font-family:'DM Mono',monospace">${f(calc.penE)}</td></tr>
${calc.rteF>0?`<tr><td>Retención en la fuente</td><td style="text-align:right">—</td><td style="text-align:right;font-family:'DM Mono',monospace">${f(calc.rteF)}</td></tr>`:""}
${calc.otrasDed>0?`<tr><td>Otras deducciones</td><td style="text-align:right">—</td><td style="text-align:right;font-family:'DM Mono',monospace">${f(calc.otrasDed)}</td></tr>`:""}
<tr style="font-weight:700;border-top:1.5px solid #111"><td>Totales</td><td style="text-align:right;font-family:'DM Mono',monospace">${f(calc.dev)}</td><td style="text-align:right;font-family:'DM Mono',monospace">${f(calc.totD)}</td></tr>
</tbody></table>
<table><tbody>
<tr><td>Neto del mes</td><td style="text-align:right;font-family:'DM Mono',monospace">${f(calc.neto)}</td></tr>
${isQ?`<tr><td>Anticipo Q1 (pagado el 15 · se descuenta)</td><td style="text-align:right;font-family:'DM Mono',monospace">${f(calc.q1)}</td></tr>
<tr style="font-weight:700;border-top:1.5px solid #111"><td>Q2 a pagar (fin de mes)</td><td style="text-align:right;font-family:'DM Mono',monospace">${f(calc.q2)}</td></tr>`:`<tr style="font-weight:700;border-top:1.5px solid #111"><td>Neto a pagar</td><td style="text-align:right;font-family:'DM Mono',monospace">${f(calc.neto)}</td></tr>`}
</tbody></table>`;
}
...
// Autorización del empleador en los justificantes de pago: recuadro como documento electrónico
// (sin firma autógrafa). Va "Administración de personal" — NO el representante legal (este solo
// firma las cartas de liquidación/preaviso/certificaciones).
function firmaEmpleadorHtml() {
  const leg = getActiveCompanyLegalDataSync();
  return `<div class="who">Por el empleador</div><div class="name">${leg.legalName||""}</div><div class="role">Administración de personal</div><div class="elec">Documento generado electrónicamente — válido sin firma autógrafa (Ley 527 de 1999)</div>`;
}
...
export function buildNominaHtml({ selN, calc, anio, mes, tipo, refBancaria }) {
  ...
  if (tipo === "anticipo") {
    ...
    <div class="cols"><div class="col"><div class="ct">EMPLEADOR</div><b>${getActiveCompanyLegalDataSync().legalName||""}</b><br/>NIT ${getActiveCompanyLegalDataSync().taxId||""}</div><div class="col"><div class="ct">TRABAJADOR(A)</div><b>${selN.nombre}</b><br/>${selN.cc}<br/>${selN.cargo}</div></div>
    <div class="contrato"><b>Contrato:</b> ${selN.tipoContrato} · Antigüedad ${antigTxt2}. <b>Anticipo Q1</b> · pago 15 de ${MESES[mes]} ${anio}.</div>
    ...
    <div class="sig"><div class="ebox">${firmaEmpleadorHtml()}</div><div class="recibi"><div class="line"></div><div class="name">${selN.nombre}</div><div class="role">Recibí conforme</div></div></div>`;
  } else {
...
// ──────────────────────────────────────────────────────────────
export function buildPrimaHtml({ selN, prima, anio, calc, incluirValores=false }) {
...
export function buildPrimaJustificanteHtml({ selN, prima, anio, refBancaria }) {
  ...
  <div class="cols"><div class="col"><div class="ct">EMPLEADOR</div><b>${leg.legalName||""}</b><br/>NIT ${leg.taxId||""}</div><div class="col"><div class="ct">TRABAJADOR(A)</div><b>${selN.nombre}</b><br/>${selN.cc}<br/>${selN.cargo}<br/>${selN.banco||"—"}${selN.tipoCuenta?" · "+selN.tipoCuenta:""}${selN.cuenta?" · "+maskCuenta(selN.cuenta):""}</div></div>
  ...
  const css = `@page{size:A5 portrait;margin:8mm}*{margin:0;padding:0;box-sizing:border-box}body{font-family:'DM Sans','Helvetica Neue',Helvetica,Arial,sans-serif;background:#e8e8e8;padding:20px 0}#content{background:#fff;width:560px;margin:0 auto;padding:40px 50px;font-size:8.5pt;color:#111;line-height:1.5;box-shadow:0 1px 4px rgba(0,0,0,.1)}.hdr{padding-bottom:12px;margin-bottom:16px;border-bottom:2px solid #111;overflow:hidden}.hdr .l{float:left}.hdr .r{float:right;text-align:right;font-size:7.5pt;color:#999;padding-top:12px}.hdr img{height:46px}h1{font-size:13pt;font-weight:700;text-align:center;margin:6px 0 4px;letter-spacing:.3px;clear:both}.sub{text-align:center;font-size:7.5pt;color:#999;margin-bottom:14px}.cols{display:flex;gap:12px;margin-bottom:10px}.col{flex:1;border:1px solid #eee;border-radius:4px;padding:8px 12px;font-size:8pt;color:#555}.ct{font-size:6.5pt;font-weight:700;color:#111;letter-spacing:.5px;margin-bottom:3px}.col b{color:#111}.contrato{font-size:8pt;color:#555;margin-bottom:8px}.contrato b{color:#111}.info{margin-bottom:14px;font-size:8pt;overflow:hidden;border:1px solid #eee;border-radius:3px;padding:8px 12px}.info div{float:left;width:50%;padding:2px 0;color:#555}.info span{color:#999}.info b{color:#333}table{width:100%;border-collapse:collapse;font-size:8pt;clear:both;margin-bottom:4px}td{padding:4px 8px;border-bottom:1px solid #f2f2f2;color:#444}.r{text-align:right;font-family:'DM Mono','SF Mono',Menlo,monospace;font-size:8pt}.liq td{border-top:2px solid #333;border-bottom:none;font-weight:700;color:#111;padding:8px;font-size:9pt}.sig{margin-top:40px;display:flex;align-items:center;justify-content:space-between;gap:24px;padding:0 6px}.sig>div{width:46%;text-align:center}.sig .ebox{border:1px solid #bbb;border-radius:6px;padding:10px 13px;background:none}.sig .who{font-size:6.5pt;font-weight:700;color:#111;letter-spacing:.4px;margin-bottom:3px}.sig .line{border-top:1px solid #999;margin-bottom:6px}.sig .name{font-size:8pt;font-weight:600;color:#333}.sig .role{font-size:7pt;color:#999;margin-top:1px}.sig .elec{font-size:6pt;color:#999;font-style:italic;margin-top:4px;line-height:1.35}.foot{font-size:6pt;color:#bbb;text-align:center;margin-top:20px;clear:both;letter-spacing:.3px}.np{text-align:center;margin:16px auto;max-width:560px}.btn{background:#333;color:#fff;border:none;padding:8px 20px;border-radius:3px;cursor:pointer;font-size:10pt;font-weight:500;margin:0 4px;font-family:inherit}@media print{body{background:#fff;padding:0}.np{display:none}#content{width:100%;margin:0;padding:8mm 10mm;box-shadow:none}}`;
