              ...
              <div key={i} style={{display:"flex",alignItems:"center",gap:14,padding:"14px 16px",background:"#FAFAF8",border:`1px solid ${T.border}`,borderRadius:8,marginBottom:8,cursor:"pointer",transition:"all .15s"}}
                onClick={()=>{
                  ...
                  if(d.action==="anticipo"){
                    ...
                    bodyHtml=`<h1>Nómina</h1><div class="sub">${MESES[mes]} ${anio} · ${selN.tipoContrato} · Período: 01 al ${new Date(anio,mes+1,0).getDate()} ${MESES[mes]} ${anio}</div>
                    ...
                    <div class="section-title">Aportes empleador (coste seguridad social empresa)</div>
                    <table class="emp"><tbody>
                    ${calc.epsEr>0?`<tr><td>EPS empleador</td><td class="r">8,5%</td><td class="r">${fmt(calc.epsEr)}</td></tr>`:`<tr><td>EPS empleador</td><td class="r" colspan="2" style="color:#999;font-style:italic">Exonerado (Art.114-1 ET)</td></tr>`}
                    <tr><td>Pensión empleador</td><td class="r">12%</td><td class="r">${fmt(calc.penEr)}</td></tr>
                    <tr><td>ARL ${ARL_OPTS[selN.arl||0]?.n||""}</td><td class="r">${(calc.tasa*100).toFixed(3)}%</td><td class="r">${fmt(calc.arlV)}</td></tr>
                    <tr><td>Caja compensación</td><td class="r">4%</td><td class="r">${fmt(calc.caja)}</td></tr>
                    ${!calc.exS?`<tr><td>SENA</td><td class="r">2%</td><td class="r">${fmt(calc.sena)}</td></tr><tr><td>ICBF</td><td class="r">3%</td><td class="r">${fmt(calc.icbf)}</td></tr>`:`<tr><td>SENA / ICBF</td><td class="r" colspan="2" style="color:#999;font-style:italic">Exonerado (Art.114-1 ET)</td></tr>`}
                    <tr class="tot"><td>Coste SS empresa</td><td class="r"></td><td class="r">${fmt(calc.totAp)}</td></tr>
                    </tbody></table>
                    ...
                    <tr><td>Salario bruto (devengado)</td><td class="r">${fmt(calc.dev)}</td><td class="r">${fmt(calc.dev*mesesYTD)}</td></tr>
                    <tr><td>Seguridad social empleado</td><td class="r">${fmt(calc.epsE+calc.penE)}</td><td class="r">${fmt((calc.epsE+calc.penE)*mesesYTD)}</td></tr>
                    ...
                    <tr><td>Coste empresa (SS)</td><td class="r">${fmt(calc.totAp)}</td><td class="r">${fmt(calc.totAp*mesesYTD)}</td></tr>
                    </tbody></table>

                    <div style="font-size:7pt;color:#bbb;margin:8px 0">IBC: ${fmt(calc.ibc)} · Ref: ${ref}</div>
