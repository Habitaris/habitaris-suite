              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                ...
                {(() => {
                  ...
                  return (
                    ...
                    <button type="button" onClick={()=>{if(!ed)return;if(!confirm("⚠️ ATENCIÓN: Vas a borrar TODO el mes de este empleado:\n- Todas las imputaciones a centros (OT)\n- Todas las novedades (incapacidades, vacaciones, licencias, ausencias)\n- TODOS los archivos adjuntos asociados (PDFs, imágenes)\n\nESTA ACCIÓN ES IRREVERSIBLE. ¿Continuar?"))return;if(!confirm("Última confirmación: vas a perder DEFINITIVAMENTE los archivos subidos del mes. ¿Seguro que quieres continuar?"))return;(async()=>{try{setGuard(true);const rn=await fetch(`/api/hiring?kv=nomina&anio=${anio}&mes=${mes}`);const dn=await rn.json();const arr=dn.data||[];const idx=arr.findIndex(n=>n.empId===selN.empId);if(idx<0){setGuard(false);throw new Error("emp");}const novActual=arr[idx].novDias||{};const tipoMap={incapacidad:"incapacidad",vacaciones:"vacaciones",licencia:"licencia_remunerada",licNoRem:"licencia_no_remunerada",ausencia:"ausencia"};for(const [fecha,tipo] of Object.entries(novActual)){const tipoAPI=tipoMap[tipo]||tipo;await fetch("/api/novelties",{method:"DELETE",headers:{"Content-Type":"application/json"},body:JSON.stringify({employee_id:arr[idx].empId,fecha_inicio:fecha,tipo:tipoAPI})}).catch(()=>{});}arr[idx].impDias={};arr[idx].novDias={};arr[idx].dias=30;arr[idx].diasIncap=0;arr[idx].diasVac=0;arr[idx].diasLicRem=0;arr[idx].diasLicNoRem=0;const rs=await fetch(`/api/hiring?kv=nomina&anio=${anio}&mes=${mes}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({data:arr})});const sd=await rs.json();if(!sd.ok){setGuard(false);throw new Error("save");}const radj=await fetch(`/api/hiring?kv=nov_adjuntos&anio=${anio}&mes=${mes}`);const dadj=await radj.json();const adj=dadj.data||{};const empPrefix=arr[idx].empId+":";const adjFiltrado={};Object.keys(adj).forEach(k=>{if(!k.startsWith(empPrefix))adjFiltrado[k]=adj[k];});await fetch(`/api/hiring?kv=nov_adjuntos&anio=${anio}&mes=${mes}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({data:adjFiltrado})});setNoms(arr);setGuard(false);window.toast&&window.toast("✓ Todo borrado: imputaciones, novedades y archivos","success");}catch(e){setGuard(false);window.toast&&window.toast("Error al borrar","error");}})();}} disabled={!ed} title="Borra TODO del mes para este empleado: imputaciones, novedades y archivos adjuntos. Irreversible." style={{padding:"4px 10px",border:`1px solid ${ed?"#dc2626":T.border}`,borderRadius:6,background:ed?"#fff":"#f5f5f5",color:ed?"#dc2626":"#999",cursor:ed?"pointer":"not-allowed",fontSize:11,fontWeight:600,fontFamily:"'DM Sans',sans-serif",marginLeft:8}}>🗑 Borrar TODO del mes</button>
                  );
                })()}
              </div>
