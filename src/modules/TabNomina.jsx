              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                ...
                {(() => {
                  ...
                  return (
                    ...
                    <button type="button" onClick={async()=>{if(!ed)return;if(!confirm("ATENCION: Vas a borrar TODO el mes (imputaciones, novedades, archivos). Es IRREVERSIBLE. Continuar?"))return;if(!confirm("Confirma de nuevo: vas a perder archivos para siempre. Seguro?"))return;try{setGuard(true);const rn=await fetch(`/api/hiring?kv=nomina&anio=${anio}&mes=${mes}`);const dn=await rn.json();const arr=dn.data||[];const idx=arr.findIndex(n=>n.empId===selN.empId);if(idx<0){setGuard(false);return;}const novAct=arr[idx].novDias||{};const tm={incapacidad:"incapacidad",vacaciones:"vacaciones",licencia:"licencia_remunerada",licNoRem:"licencia_no_remunerada",ausencia:"ausencia"};for(const fe in novAct){await fetch("/api/novelties",{method:"DELETE",headers:{"Content-Type":"application/json"},body:JSON.stringify({employee_id:arr[idx].empId,fecha_inicio:fe,tipo:tm[novAct[fe]]||novAct[fe]})}).catch(()=>{});}arr[idx].impDias={};arr[idx].novDias={};arr[idx].dias=30;arr[idx].diasIncap=0;arr[idx].diasVac=0;arr[idx].diasLicRem=0;arr[idx].diasLicNoRem=0;const rs=await fetch(`/api/hiring?kv=nomina&anio=${anio}&mes=${mes}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({data:arr})});const sd=await rs.json();if(!sd.ok){setGuard(false);return;}const radj=await fetch(`/api/hiring?kv=nov_adjuntos&anio=${anio}&mes=${mes}`);const dadj=await radj.json();const adj=dadj.data||{};const pre=arr[idx].empId+":";const adjFil={};Object.keys(adj).forEach(k=>{if(!k.startsWith(pre))adjFil[k]=adj[k];});await fetch(`/api/hiring?kv=nov_adjuntos&anio=${anio}&mes=${mes}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({data:adjFil})});setNoms(arr);setGuard(false);window.toast&&window.toast("Todo borrado del mes","success");}catch(e){setGuard(false);window.toast&&window.toast("Error al borrar","error");}}} disabled={!ed} title="Borra todo del mes: imputaciones, novedades y archivos. Irreversible." style={{padding:"4px 10px",border:`1px solid ${ed?"#dc2626":T.border}`,borderRadius:6,background:ed?"#fff":"#f5f5f5",color:ed?"#dc2626":"#999",cursor:ed?"pointer":"not-allowed",fontSize:11,fontWeight:600,fontFamily:"'DM Sans',sans-serif",marginLeft:8}}>🗑 Borrar TODO del mes</button>
                  );
                })()}
              </div>
