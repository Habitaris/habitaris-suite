...
const EstadoPills=({n,showPrima,primaPaid})=>{
  ...
  const primaPill = showPrima ? <span key="pr" style={pillStyle(primaPaid)}>{primaPaid?"✓ Prima":"Prima"}</span> : null;
  if(n.modalidadPago==="mensual"){
    const paid=n.estado==="pagada"||n.estado==="liquidada";
    return <div style={{display:"flex",gap:4}}><span style={pillStyle(paid)}>{paid?"✓ Mes":"Mes"}</span>{primaPill}</div>;
  }
  // Quincenal
  const q1Paid=n.estado==="q1_pagado"||n.estado==="pagada"||n.estado==="liquidada";
  const q2Paid=n.estado==="pagada"||n.estado==="liquidada";
  return <div style={{display:"flex",gap:4}}><span style={pillStyle(q1Paid)}>{q1Paid?"✓ Q1":"Q1"}</span><span style={pillStyle(q2Paid)}>{q2Paid?"✓ Q2":"Q2"}</span>{primaPill}</div>;
};
...
.sig{margin-top:36px;overflow:hidden}.sig div{float:left;width:48%;text-align:center;font-size:8pt;border-top:1px solid #111;padding-top:6px}.sig div:last-child{float:right}.sig div.auth{border-top:none;padding-top:0}
.authbox{border:1px solid #bbb;border-radius:6px;padding:12px 18px;margin-top:24px;font-size:8.5pt;color:#555;text-align:center;background:#FAFAF9;clear:both}
.authbox .who{font-size:7pt;font-weight:700;color:#111;text-transform:uppercase;letter-spacing:.6px;margin-bottom:5px}
.authbox b{color:#111}
.recibi{margin-top:30px;width:46%;text-align:center;font-size:8pt;border-top:1px solid #111;padding-top:6px;clear:both}
          ...
          <Card accent={T.ink}>
            ...
            {[
              ...
              <button key={d.id} onClick={()=>{
                ...
                const _rep = _leg.legalRep;
                // Autorización del empleador: datos del representante legal + validez como documento
                // electrónico (sin firma autógrafa). El representante NO firma físicamente.
                const _firmaEmp = (_rep && _rep.name)
                  ? `<b>${_rep.name}</b><br/><span style="font-size:7.5pt;color:#555">${_rep.cargo||"Representante Legal"}${_rep.document_number?` · ${_rep.document_type||"CC"} ${_rep.document_number}`:""}</span><br/><span style="font-size:7.5pt;color:#555">${_leg.legalName} · NIT ${_leg.taxId}</span><br/><span style="font-size:6.5pt;color:#999;font-style:italic">Documento generado electrónicamente — válido sin firma autógrafa (Ley 527 de 1999)</span>`
                  : `<b>${_leg.legalName}</b><br/>NIT: ${_leg.taxId}<br/><span style="font-size:6.5pt;color:#999;font-style:italic">Documento generado electrónicamente — válido sin firma autógrafa (Ley 527 de 1999)</span>`;
                if(d.id==="preaviso"){
...
<button class="btn" onclick="window.print()">🖨 Imprimir / Guardar como PDF</button>
    ...
    const genImputaciónesHtml = async () => {
            ...
            // Por centro: días trabajados + festivos + novedades. Las novedades NO
            // remuneradas (ausencia, lic. no rem.) restan; el resto (incapacidad,
            // vacaciones, licencia rem.) suman. El % reparte la prima del semestre.
            if (mes === 5 || mes === 11) {
    ...
    return(
      <div className="fade-up" style={{maxWidth:1050,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:8,flexWrap:"wrap"}}>
          ...
          <EstadoPills n={selN} showPrima={mes===5||mes===11} primaPaid={!!(soportesPago[selN.empId+":prima"]&&soportesPago[selN.empId+":prima"].data)}/>
        ...
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          ...
          <tbody>{sorted.length===0?<tr><td colSpan={11} style={{padding:24,textAlign:"center",color:T.inkLight,fontSize:12}}>Sin resultados para "{buscar}"</td></tr>:sorted.map(n=>{const c=calcN(n);return(
            <tr key={n.id} style={{borderTop:`1px solid ${T.border}`,cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.background=T.accent} onMouseLeave={e=>e.currentTarget.style.background=""}>
              ...
              <td style={{padding:"9px 12px"}}><EstadoPills n={n} showPrima={mes===5||mes===11} primaPaid={!!(soportesPago[n.empId+":prima"]&&soportesPago[n.empId+":prima"].data)}/></td>
