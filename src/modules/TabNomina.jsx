          ...
          <Card accent={T.ink}>
            ...
            {[
              ...
              <button key={d.id} onClick={()=>{
                ...
                // Firma del empleador: datos impresos del representante legal (sin firma manuscrita).
                const _firmaEmp = (_rep && _rep.name)
                  ? `<b>${_rep.name}</b><br/><span style="font-size:7.5pt;color:#555">${_rep.cargo||"Representante Legal"}${_rep.document_number?` · ${_rep.document_type||"CC"} ${_rep.document_number}`:""}</span><br/><span style="font-size:7.5pt;color:#555">${_leg.legalName} · NIT ${_leg.taxId}</span>`
                  : `<b>${_leg.legalName}</b><br/>NIT: ${_leg.taxId}`;
                if(d.id==="preaviso"){
